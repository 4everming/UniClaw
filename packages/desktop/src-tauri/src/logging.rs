...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("uniclaw-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("uniclaw_lib=debug,uniclaw_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("uniclaw_lib=info,uniclaw_desktop=info,sidecar=info")
        }
    });
